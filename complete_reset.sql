...
-- מחיקה מכל הטבלאות התלויות ב-jobs
DELETE FROM job_files WHERE job_id IN (SELECT id FROM jobs WHERE org_id = 3);
DELETE FROM delivery_notes WHERE job_id IN (SELECT id FROM jobs WHERE org_id = 3);
DELETE FROM weigh_tickets WHERE job_id IN (SELECT id FROM jobs WHERE org_id = 3);
DELETE FROM job_status_events WHERE org_id = 3;
DELETE FROM jobs WHERE org_id = 3;

-- מחיקת price_lists שמקושרות לאתרים
DELETE FROM price_lists WHERE org_id = 3;
...
-- מחיקת users של נהגים (רק אלו עם @demo.com)
DELETE FROM users WHERE org_id = 3 AND email LIKE '%@demo.com';
